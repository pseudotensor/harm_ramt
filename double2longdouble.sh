...
for fil in ls *.c *.h makefile; do
  echo $fil;
  #diff -bBdp $fil $1/$fil;
  sed -e 's/%g/%Lg/g' -e 's/%21.15g/%26.20Lg/g' -e 's/%12.15g/%26.20Lg/g' -e 's/%15.10g/%26.20Lg/g' -e 's/%10.5g/%26.20Lg/g' -e 's/%26.20g/%26.20Lg/g' -e 's/%26.20e/%26.20Lg/g' -e 's/%20.10g/%26.20Lg/g' -e 's/%25.17g/%26.20Lg/g' -e 's/%15.8g/%26.20Lg/g' -e 's/%lf/%Lf/g' -e 's/sqrt(/sqrtl(/g' -e 's/fabs(/fabsl(/g' -e 's/sin(/sinl(/g' -e 's/cos(/cosl(/g' -e 's/tan(/tanl(/g' -e 's/atan(/atanl(/g' -e 's/pow(/powl(/g' -e 's/M_PI/M_PIl/g' -e 's/M_PIll/M_PIl/g' -e 's/#define CTYPE double/#define CTYPE long double/g' -e 's/#define MPI_CTYPE MPI_DOUBLE/#define MPI_CTYPE MPI_LONG_DOUBLE/g' -e 's/#define REALTYPE DOUBLETYPE/#define REALTYPE LONGDOUBLETYPE/g' -e 's/#define SENSITIVE DOUBLETYPE/#define SENSITIVE LONGDOUBLETYPE/g' -e 's/#define SUPERLONGDOUBLE 0/#define SUPERLONGDOUBLE 1/g'  $fil > $fil.new
  #
