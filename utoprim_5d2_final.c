...
FTYPE A_orig[NEWT_DIM][NEWT_DIM];
/******************************************************************
...
int Utoprim_5d2_final(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  if( ret ){
    ...
    if( ret_gam == 1 ) { 
      pflag[icurr][jcurr][FLAGUTOPRIMFAIL]= UTOPRIMFAILCONVW;
  ...
  pflag[icurr][jcurr][FLAGUTOPRIMFAIL]= UTOPRIMNOFAIL;
...
#if(!OPTIMIZED)
  if(failreturn>=1)
    FAILSTATEMENTVOID("func_5d2()", "get_state()");
#endif
...
#if(!OPTIMIZED)
  if(failreturn>=1)
    FAILSTATEMENTVOID("utoprim.c:usrfun2()", "get_state()");
#endif
...
#if(!OPTIMIZED)
  if(failreturn>=1)
    FAILSTATEMENTVOID("utoprim.c:usrfun2()", "dudp_calc()");
#endif
...
// jon 04-24-2003 designed for relative 4-velocity
...
static int dudp_calc_g(FTYPE *pr, struct of_state *q, struct of_geom *geom, FTYPE Am[][NEWT_DIM] )
{
	...
	static void dutdui_calc_g(FTYPE *ucon,FTYPE *dutdui) ;

	static void dbtdui_calc_g(FTYPE *dutdui, FTYPE *pr, FTYPE *dbtdui) ;
	static void dbiduj_calc_g(FTYPE *dbtdui,FTYPE *dutdui,FTYPE *ucon, FTYPE *b, FTYPE dbiduj[][NDIM]) ;
	static void dbsqdui_calc_g(FTYPE dbiduj[][NDIM],FTYPE *b, FTYPE *dbsqdui) ;
	static FTYPE contract_g(FTYPE *vcon1, FTYPE *vcon2);
	static FTYPE covtract_g(FTYPE *vcov1, FTYPE *vcov2);
	static void duudud_calc_g(FTYPE *ucon, FTYPE duudud[][NDIM]); 
	static void dudduu_calc_g(FTYPE*dutdui, FTYPE dudduu[][NDIM]) ;
	static void dbdiduj_calc_g(FTYPE dbiduj[][NDIM],FTYPE dbdiduj[][NDIM]);
	static void ducon_dv3_calc_g(struct of_state *q,FTYPE ducon_dv3[][NDIM]);



	ptrlgeom=geom; // so don't have to rewrite functions yet
	...
	// d u^t / d u^i
	dutdui_calc_g(q->ucon,dutdui) ;
	// d b^t / d u^i
	dbtdui_calc_g(dutdui,pr,dbtdui) ;
	// d b^{mu} / d u^j
	dbiduj_calc_g(dbtdui,dutdui,q->ucon,q->bcon,dbiduj) ;
	// d b^2 / d u^i
	dbsqdui_calc_g(dbiduj,q->bcon,dbsqdui) ;
	// d u^{mu} / d u^j
	dudduu_calc_g(dutdui,dudduu);
	// d b_{mu} / d u^j
	dbdiduj_calc_g(dbiduj,dbdiduj);

	eta = pr[RHO] + gam*pr[UU] + bsq ;
	...
	// rho on rho
	Am[RHO][RHO] = q->ucon[TT] ;
	// u+momentums on rho
	DLOOPA Am[UU+j][RHO] = q->ucon[TT]*q->ucov[j];

	// U[rho] on u
	Am[RHO][UU] = 0.0;
	// u+momentums on u
	DLOOPA Am[UU+j][UU] = gam*q->ucon[TT]*q->ucov[j]+delta(TT,j)*(gam-1.0);
	...
	/* this bit of legacy code can be uncommented if
	   ...
	   flux, which may be numerically convenient (tests are inconclusive) */
	...
	*/
	...
	// change of variables
	  ...
	  // then we are done!
#elif(WHICHVEL==VEL3)
	  ducon_dv3_calc_g(q,ducon_dv3);

	  // convert to relative velocity (over all A[U=RHO->UTCON3][p=v1,v2,v3])
	  // 4vel -> 3vel
	  // assign parts that don't change variable
	  for(i=0; i < NEWT_DIM; i++) for(j=RHO;j<=UU;j++) tempA[i][j] = Am[i][j];
	  for(i=0; i < NEWT_DIM; i++) SLOOPA for(l=1; l<=3; l++) tempA[RHO+i][UTCON1+j-1] += Am[RHO+i][UTCON1+l-1]*ducon_dv3[l][j];
	  // assign back to Am
	  ...
	  // 4vel -> rel4vel
	  for(i=0; i < NEWT_DIM; i++) for(j=RHO; j<=UU; j++) tempA[i][j] = Am[i][j];
	  for(i=0; i < NEWT_DIM; i++) SLOOPA for(l=1;l<=3;l++) tempA[RHO+i][UTCON1+j-1] += Am[RHO+i][UTCON1+l-1]*duidvj[l][j];
	  for(i=0; i < NEWT_DIM; i++) for(j=0; j < NEWT_DIM; j++) Am[i][j] = tempA[i][j];
#endif
	...
	// Am is dU^i/dp^j = Am[i][j]
	  ...
	  //	  for( j=0; j < NEWT_DIM; j++)
	  //	    for( k = 0; k < NEWT_DIM; k++) Am[j][k] *= geom->g ;
	  //#elif(WHICHEOM==WITHNOGDET)
	  // only the U^t (dU^2/dp^j) and U^\phi (dU^5/dp^j) contain gdet
	  //	  for( j = 0; j < NEWT_DIM; j++){
	  //	    if(j==0) for(k = 0; k < NEWT_DIM ; k++) Am[j][k] *= geom->g ;
	  //	    else if((j==1)&&(NOGDET0==0)) for( k = 0; k < NEWT_DIM ; k++ )  Am[j][k] *= geom->g ;
	  //	    else if((j==2)&&(NOGDET1==0)) for( k = 0; k < NEWT_DIM ; k++ )  Am[j][k] *= geom->g ;
	  //	    else if((j==3)&&(NOGDET2==0)) for( k = 0; k < NEWT_DIM ; k++ )  Am[j][k] *= geom->g ;
	  //	    else if((j==4)&&(NOGDET3==0)) for( k = 0; k < NEWT_DIM ; k++ )  Am[j][k] *= geom->g ;
	  //	  }
	  //#endif
...
static void dgdvi_calc_g(FTYPE *pr,FTYPE *dgdvi)
{
  ...
  gamma_calc(pr,ptrlgeom,&gamma) ;

  SLOOPA dgdvi[j]=0.0;

  SLOOP dgdvi[j]+=1.0/gamma*ptrlgeom->gcov[j][k]*pr[UTCON1+k-1];
...
static void duidvj_calc_g(FTYPE *dgdvi,FTYPE duidvj[][NDIM])
{
  ...
  alpha=1.0/sqrt(-ptrlgeom->gcon[TT][TT]);
  SLOOPA betacon[j]=ptrlgeom->gcon[TT][j]*alpha*alpha;
...
static FTYPE contract_g(FTYPE *vcon1, FTYPE *vcon2)
{
	...
	n = 0. ;
	DLOOP n += ptrlgeom->gcov[j][k]*vcon1[j]*vcon2[k] ;
	return(n) ;
...
static FTYPE covtract_g(FTYPE *vcov1, FTYPE *vcov2)
{
	...
	n = 0. ;
	DLOOP n += ptrlgeom->gcon[j][k]*vcov1[j]*vcov2[k] ;
	return(n) ;
...
static void duudud_calc_g(FTYPE *ucon, FTYPE duudud[][NDIM]) 
{
  ...
  DSLOOP duudud[j][k] = ptrlgeom->gcon[j][k] + ptrlgeom->gcon[j][TT]*(-ucon[k]/ucon[TT]) ;
}
...
static void dudduu_calc_g(FTYPE*dutdui, FTYPE dudduu[][NDIM]) 
{
  ...
  DSLOOP dudduu[j][k] = ptrlgeom->gcov[j][k] + ptrlgeom->gcov[j][TT]*dutdui[k];
...
static void dbdiduj_calc_g(FTYPE dbiduj[][NDIM],FTYPE dbdiduj[][NDIM])
{
  ...
  DSLOOP for(l=0;l<NDIM;l++) dbdiduj[j][k] += ptrlgeom->gcov[j][l]*dbiduj[l][k];
...
static int LU_decompose( FTYPE A[][NEWT_DIM], int permute[] )
{
  ...
  for( i = 0; i < n; i++ ) { 
    ...
    if( absmax == 0. ) { 
      dualfprintf(fail_file, "LU_decompose(): row-wise singular matrix: %d %d\n", icurr, jcurr);
//      for(k=0;k<n;k++) { 
  ...
  */
  ...
  for( j = 0; j < n; j++ ) {
    ...
    */
    ...
    if( A[j][j] == 0. ) { 
      A[j][j] = absmin;
      //dualfprintf(fail_file, "LU_decompose(): column-wise singular matrix: %d %d\n", icurr, jcurr);
      //return(1);
    }
