...
static int Utoprim_new_body(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  if( (ret != 0) || (W == FAIL_VAL) ) {
    ...
    pflag[ptrgeom->i][ptrgeom->j][FLAGUTOPRIMFAIL]= ret*100+1;// related to UTOPRIMFAILCONVRET
    return(0);
  }
  else{
    Wtest=W/wglobal;
			    ...
			    FTYPE (*res_func) (FTYPE []) )
{
  ...
  while( keep_iterating ) { 
    ...
    if( is_nan_inf( f ) || is_nan_inf( df ) ) {
      fprintf(stderr,"general_newton_raphson(): nan encountered in f or df!! \n");
      return(1);
    }
  ...
  *check=0;
  ...
  for (;;) {
    ...
    *f=(*func)(x+1);
    ...
    if( bad_step ) { 
      *check = 2;
      fprintf(stderr,"my_lnsrch(): bad_step = 1,  f = %21.15g \n", *f); fflush(stderr);
      return;
    }
	...
	if (a == 0.0) tmplam = -slope/(2.0*b);
	  ...
	  if (disc<0.0) {
	    if( disc < -1.e-10 ) {
	      fprintf(stderr,"my_lnsrch(): Big Roundoff problem:  disc = %21.15g \n", disc);
	      fflush(stderr);
	    }
	    disc = 0.;
	  }
