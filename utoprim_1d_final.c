...
int Utoprim_1d_final(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
...
/*   // only output if failed */
/*   if(pflag[ptrgeom->i][ptrgeom->j][FLAGUTOPRIMFAIL]&&(debugfail>=1)) dualfprintf(fail_file, "utoprim_dbl2(): Failed to find a prim. var. solution!! t=%21.15g nstep=%ld i=%d j=%d\n",t,nstep,ptrgeom->i,ptrgeom->j); */
...
pflag[ptrgeom->i][ptrgeom->j]:  (i*100 + j)  where 
...
static int Utoprim_new_body(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  */
  ...
  if( (retval != 0) || (W == FAIL_VAL) ) {
    ...
    pflag[ptrgeom->i][ptrgeom->j][FLAGUTOPRIMFAIL]=  retval*100+1;
    return(retval);
  }
  else{
